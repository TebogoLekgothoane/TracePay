...
type FrozenItem = Awaited<ReturnType<typeof apiClient.listFrozen>>[number];
...
export default function MyLeaksPage() {
  ...
  async function load() {
    try {
      ...
      const [analyses, frozenItems] = await Promise.all([
        apiClient.getMyAnalyses(1),
        apiClient.listFrozen(),
      ]);
      setLatest(analyses[0] ?? null);
      setFrozen(frozenItems);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Could not load your leaks.");
    } finally {
      setLoading(false);
    }
  }
  ...
  if (loading) {
    return <div className="p-6 text-sm text-muted-foreground">Loading your leaks...</div>;
  }
  ...
  return (
    <div className="space-y-6 p-2 md:p-4">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">My Leaks</h1>
        <p className="text-sm text-muted-foreground">
          {latest
            ? `From your last analysis, ${new Date(latest.created_at).toLocaleDateString()}`
            : "Run an analysis from the Health Score page to see your leaks here."}
        </p>
      </div>
      ...
      {leaks.length === 0 ? (
        <p className="text-sm text-muted-foreground">No leaks found in your latest analysis.</p>
      ) : (
