...
async function request<T>(
  ...
  if (!response.ok) {
    ...
    throw new Error(err.error?.message || err.detail || "Backend request failed");
  }
...
function versionedPath(path: string): string {
  ...
  return /^\/v\d+(\/|$)/.test(cleanPath) ? cleanPath : `${API_VERSION_PREFIX}${cleanPath}`;
}
...
export interface BackendAuthActionResponse {
  message: string;
}
...
export async function loginWithBackend(
  ...
  const url = `${NORMALIZED_BACKEND_BASE_URL}${versionedPath("/auth/login")}`;
...
export async function registerWithBackend(
  ...
  const url = `${NORMALIZED_BACKEND_BASE_URL}${versionedPath("/auth/register")}`;
  const response = await fetch(url, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ email, password }),
...
export async function requestPasswordResetWithBackend(
  email: string
): Promise<BackendAuthActionResponse> {
...
export async function confirmPasswordResetWithBackend(
  token: string,
  newPassword: string
...
export async function requestEmailVerificationWithBackend(
  email: string
): Promise<BackendAuthActionResponse> {
...
export async function confirmEmailVerificationWithBackend(
  token: string
): Promise<{ message: string }> {
